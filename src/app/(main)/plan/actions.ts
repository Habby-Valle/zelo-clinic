...
export async function arePlansEnabled(): Promise<boolean> {
  try {
    const data = await apiFetchServer<Record<string, unknown>>(
...
export async function requestPlanChange(
  ...
  try {
    const data = await apiFetchServer<DjangoPlanList>(
      `/plans/?page_size=100&scope=clinic`
    )
    targetPlan =
      (data.results ?? []).find((p) => String(p.id) === planId) ?? null
  } catch {
...
export async function manageGetClinic(): Promise<{
  ...
  try {
    const [clinicData, customerData] = await Promise.all([
      ...
      apiFetchServer<{ stripe_customer_id: string | null }>(
