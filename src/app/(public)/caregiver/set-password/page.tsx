...
function CaregiverSetPasswordContent() {
  ...
  const deepLink = `zeloapp://caregiver/set-password?${query}`;
  const pageUrl = typeof window !== "undefined" ? window.location.href : "";

  const androidIntentUrl = `intent://caregiver/set-password?${query}#Intent;scheme=zeloapp;package=${APP_PACKAGE};S.browser_fallback_url=${encodeURIComponent(pageUrl || PLAY_STORE_URL)};end;`;
    ...
    <div className="flex flex-col items-center justify-center gap-8 py-12 text-center">
      ...
      <div className="space-y-2">
        <h1 className="text-2xl font-bold tracking-tight">
          Voc&ecirc; foi cadastrado no <span className="text-primary">Zelo</span>
        </h1>
        <p className="text-muted-foreground">
          Abra o aplicativo para criar sua senha e come&ccedil;ar a usar.
        </p>
      </div>
      ...
      <div className="space-y-3 rounded-lg border bg-muted/30 p-6">
        ...
        <p className="text-xs text-muted-foreground">
          Depois de baixar, abra o link novamente para criar sua senha no app.
        </p>
      </div>
    </div>
  );
...
export default function CaregiverSetPasswordPage() {
  return (
    <Suspense
      fallback={
        <div className="flex flex-col items-center justify-center gap-4 py-24 text-center">
