...
export async function fetchClinicPatientsApi(): Promise<PatientOption[]> {
  ...
  return data.results.map((p) => ({
    ...
    contract_preferred_weekdays: (p.contract_preferred_weekdays as number[] | null) ?? null,
    contract_preferred_start_time: (p.contract_preferred_start_time as string | null) ?? null,
    contract_preferred_end_time: (p.contract_preferred_end_time as string | null) ?? null,
  }));
}
