...
export async function fetchHealthAlertsApi(filters: HealthAlertFilters): Promise<HealthAlert[]> {
  ...
  return apiFetchClient<HealthAlert[]>(`/api/ai/health-alerts/?${qs.toString()}`);
}

export async function acknowledgeHealthAlertApi(id: string): Promise<void> {
  await apiFetchClient<void>(`/api/ai/health-alerts/${id}/acknowledge/`, { method: "POST" });
}

export async function resolveHealthAlertApi(id: string): Promise<void> {
  await apiFetchClient<void>(`/api/ai/health-alerts/${id}/resolve/`, { method: "POST" });
}
